...
{% endif %}
    <tbody>
        <tr>
            ...
            {% if profileRole == 'A' %}
            <td width="12%">{{ link_to("profiles/edit/" ~ profile._id, '<i class="icon-pencil"></i> Edit', "class": "btn") }}</td>
            <td width="12%">{{ link_to("profiles/delete/" ~ profile._id, '<i class="icon-remove"></i> Delete', "class": "btn") }}</td>
            {% endif %}
        </tr>
    </tbody>
{% if loop.last %}
